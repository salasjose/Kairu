import type { CrosswordData } from './types';

export const SUSTAINABILITY_PUZZLE: CrosswordData = {
  title: "Transformación Sostenible",
  grid: [
    ["S","O","S","T","E","N","I","B","I","L","I","D","A","D","#"],
    ["#","#","#","#","#","#","#","#","#","#","#","#","#","#","T"],
    ["D","E","S","A","R","R","O","L","L","O","#","#","#","#","R"],
    ["#","#","#","#","#","#","#","#","#","#","R","#","#","#","A"],
    ["#","#","#","#","#","#","#","#","#","#","E","#","#","#","N"],
    ["#","#","#","#","#","#","#","#","#","#","S","#","#","#","S"],
    ["R","E","N","O","V","A","B","L","E","#","I","#","#","#","F"],
    ["#","#","#","#","#","#","#","#","#","#","L","#","#","#","O"],
    ["R","E","U","T","I","L","I","Z","A","R","I","#","#","#","R"],
    ["#","#","#","#","#","#","#","#","#","#","E","#","#","#","M"],
    ["#","E","C","O","E","F","I","C","I","E","N","C","I","A","A"],
    ["#","#","#","#","#","#","#","#","#","#","C","#","#","#","C"],
    ["E","C","O","N","O","M","I","A","#","#","I","#","#","#","I"],
    ["#","#","#","#","#","#","#","#","#","#","A","#","#","#","O"],
    ["C","O","N","C","I","E","N","C","I","A","#","#","#","#","N"],
  ],
  across: [
    { number: 1, clue: "Uso responsable de los recursos naturales para satisfacer las necesidades actuales sin comprometer las futuras.", answer: "SOSTENIBILIDAD" },
    { number: 2, clue: "Proceso que busca equilibrar lo económico, lo social y lo ambiental.", answer: "DESARROLLO" },
    { number: 3, clue: "Energía obtenida de fuentes como el sol, el viento o el agua.", answer: "RENOVABLE" },
    { number: 4, clue: "Acción de volver a utilizar un producto o material para alargar su vida útil.", answer: "REUTILIZAR" },
    { number: 5, clue: "Práctica que reduce el consumo de materiales y energía.", answer: "ECOEFICIENCIA" },
  ],
  down: [
    { number: 9, clue: "Cambio positivo hacia un modelo que respeta el medio ambiente y promueve la equidad social.", answer: "TRANSFORMACION" },
    { number: 10, clue: "Capacidad de mantener el equilibrio ecológico y social a largo plazo.", answer: "RESILIENCIA" },
    { number: 11, clue: "Sistema que permite transformar residuos en nuevos productos.", answer: "RECICLAJE" },
    { number: 12, clue: "Dimensión que equilibra recursos para el bienestar y la sostenibilidad.", answer: "ECONOMIA" },
    { number: 13, clue: "Valor que impulsa a cuidar el planeta y actuar de forma responsable con el entorno.", answer: "CONCIENCIA" },
  ],
};
